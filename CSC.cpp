...
ImageYUV CSC(ImageRGB &rgb) {
  ...
  for (int y = 0; y < rgb.getHeight(); y++) {
    for (int x = 0; x < rgb.getWidth(); x++) {
      Y = (0.2568 * rgb.at(y, x).R + 0.5041 * rgb.at(y, x).G +
           0.0979 * rgb.at(y, x).B) +
          16;
      U = (-0.1479 * rgb.at(y, x).R - 0.2896 * rgb.at(y, x).G +
           0.4375 * rgb.at(y, x).B) +
          128;
      V = (0.4375 * rgb.at(y, x).R - 0.3666 * rgb.at(y, x).G -
           0.0709 * rgb.at(y, x).B) +
          128;
      if (Y < 0)
        Y = 0;
...
ImageRGB CSC(ImageYUV &yuv) {
  ...
  for (int y = 0; y < yuv.getHeight(); y++) {
    for (int x = 0; x < yuv.getWidth(); x++) {
      ...
      G = (1.1644 * yuv.at(y, x).Y - 0.3928 * yuv.at(y, x).U -
           0.8163 * yuv.at(y, x).V + 136.1381);
