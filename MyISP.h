...
void DPC(ImageRaw &raw, uint16_t threshold = 30,
         DPC_MODE mode = DPC_MODE_GRADIENT, uint16_t clip = 1023);
// black level correction
void BLC(ImageRaw &raw, uint16_t r = 0, uint16_t gr = 0, uint16_t gb = 0,
         uint16_t b = 0, float alpha = 0, float beta = 0,
         BAYER_PATTERN bayer_pattern = BAYER_PATTERN_RGGB,
         uint16_t clip = 1023);
// lens shading correction
void LSC(ImageRaw &raw, uint16_t intensity = 0, int minR = -1, int maxR = -1,
         uint16_t clip = 1023);
...
// auto white balance
void AWB(ImageRaw &raw, float r_gain = 1.5, float gr_gain = 1.0,
         float gb_gain = 1.0, float b_gain = 1.1,
         BAYER_PATTERN bayer_pattern = BAYER_PATTERN_RGGB,
         uint16_t clip = 1023);
...
void CNF(ImageRaw &raw, BAYER_PATTERN bayer_pattern = BAYER_PATTERN_RGGB,
         float threshold = 0, float r_gain = 1.5, float gr_gain = 1.0,
         float gb_gain = 1.0, float b_gain = 1.1, uint16_t clip = 255);

// Demosaicing
ImageRGB CFA(ImageRaw &raw, DEMOSAICING_MODE cfa_mode = DEMOSAIC_MALVAR,
             BAYER_PATTERN bayer_pattern = BAYER_PATTERN_RGGB,
             uint8_t cfa_clip = 255);
