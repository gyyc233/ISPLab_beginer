...
#define TRACE_DEBUG_LOG_WARNING                                                \
  (printf("[WARNING] %s(%d)-<%s>: ", __FILE__, __LINE__, __FUNCTION__), printf)
#define TRACE_DEBUG_LOG_ERROR                                                  \
...
uint16_t &ImageRaw::at(int h, int w) { return data[width * h + w]; }
...
void ImageRaw::padding(int round, PADDING_MODE mode, uint16_t value) {
  uint16_t *dst =
      new uint16_t[(this->width + round * 2) * (this->height + round * 2)];
  switch (mode) {
  case PADDING_MODE_CONSTANT:
    for (int y = -round; y < this->height + round; y++) {
      for (int x = -round; x < this->width + round; x++) {
        if (y < 0 || y >= this->height || x < 0 || x >= this->width)
          dst[(y + round) * (this->width + round * 2) + (x + round)] = value;
        else
          dst[(y + round) * (this->width + round * 2) + (x + round)] =
              this->at(y, x);
      }
    }
    break;
    ...
    for (int y = -round; y < this->height + round; y++) {
      for (int x = -round; x < this->width + round; x++) {
        ...
        if (dst_y < 0)
          dst_y = -dst_y;
        if (dst_y >= this->height)
          dst_y = this->height * 2 - dst_y - 2;
        if (dst_x < 0)
          dst_x = -dst_x;
        if (dst_x >= this->width)
          dst_x = this->width * 2 - dst_x - 2;
        dst[(y + round) * (this->width + round * 2) + (x + round)] =
            this->at(dst_y, dst_x);
      }
    }
...
const int ImageRaw::getWidth() { return this->width; }

const int ImageRaw::getHeight() { return this->height; }

const uint16_t *ImageRaw::getData() { return this->data; }
...
PixelRGB &ImageRGB::at(int h, int w) { return data[width * h + w]; }
...
void ImageRGB::clip(uint8_t min, uint8_t max) {
  ...
  for (int y = 0; y < this->height; y++) {
    for (int x = 0; x < this->width; x++) {
      if (this->at(y, x).R > max)
        this->at(y, x).R = max;
      if (this->at(y, x).G > max)
        this->at(y, x).G = max;
      if (this->at(y, x).B > max)
        this->at(y, x).B = max;

      if (this->at(y, x).R < min)
        this->at(y, x).R = min;
      if (this->at(y, x).G < min)
        this->at(y, x).G = min;
      if (this->at(y, x).B < min)
        this->at(y, x).B = min;
    }
  }
}
...
void ImageRGB::padding(int round, PADDING_MODE mode, PixelRGB value) {
  PixelRGB *dst =
      new PixelRGB[(this->width + round * 2) * (this->height + round * 2)];
  switch (mode) {
  case PADDING_MODE_CONSTANT:
    for (int y = -round; y < this->height + round; y++) {
      for (int x = -round; x < this->width + round; x++) {
...
const int ImageRGB::getWidth() { return this->width; }

const int ImageRGB::getHeight() { return this->height; }

const PixelRGB *ImageRGB::getData() { return this->data; }
...
void ImageRGB::print() {
  for (int y = 0; y < this->getHeight(); y++) {
    for (int x = 0; x < this->getWidth(); x++) {
      printf("(%d,%d,%d) ", this->at(y, x).R, this->at(y, x).G,
             this->at(y, x).B);
    }
    printf("\n");
  }
...
PixelYUV &ImageYUV::at(int h, int w) { return data[width * h + w]; }
...
void ImageYUV::clip(uint8_t min, uint8_t max) {
  ...
  for (int y = 0; y < this->height; y++) {
    for (int x = 0; x < this->width; x++) {
      if (this->at(y, x).Y > max)
        this->at(y, x).Y = max;
      if (this->at(y, x).U > max)
        this->at(y, x).U = max;
      if (this->at(y, x).V > max)
        this->at(y, x).V = max;

      if (this->at(y, x).Y < min)
        this->at(y, x).Y = min;
      if (this->at(y, x).U < min)
        this->at(y, x).U = min;
      if (this->at(y, x).V < min)
        this->at(y, x).V = min;
    }
  }
}
...
void ImageYUV::padding(int round, PADDING_MODE mode, PixelYUV value) {
  PixelYUV *dst =
      new PixelYUV[(this->width + round * 2) * (this->height + round * 2)];
  switch (mode) {
  case PADDING_MODE_CONSTANT:
    for (int y = -round; y < this->height + round; y++) {
      for (int x = -round; x < this->width + round; x++) {
...
const int ImageYUV::getWidth() { return this->width; }

const int ImageYUV::getHeight() { return this->height; }

const PixelYUV *ImageYUV::getData() { return this->data; }
...
void ImageYUV::print() {
  for (int y = 0; y < this->getHeight(); y++) {
    for (int x = 0; x < this->getWidth(); x++) {
      printf("(%d,%d,%d) ", this->at(y, x).Y, this->at(y, x).U,
             this->at(y, x).V);
    }
    printf("\n");
  }
