...
PixelRGB malvar(const char *is_color, float center, int y, int x, ImageRaw *img,
    ...
    b = 6 * img->at(y, x) -
        3 *
            (img->at(y - 2, x) + img->at(y, x - 2) + img->at(y + 2, x) +
             img->at(y, x + 2)) /
            2 +
        2 * (img->at(y - 1, x - 1) + img->at(y - 1, x + 1) +
             img->at(y + 1, x - 1) + img->at(y + 1, x + 1));
    g = g / 8;
    b = b / 8;
  }
  if (strcmp(is_color, "gr") == 0) {
    r = 5 * img->at(y, x) - img->at(y, x - 2) - img->at(y - 1, x - 1) -
        ...
        img->at(y, x + 2) + (img->at(y - 2, x) + img->at(y + 2, x)) / 2 +
        4 * (img->at(y, x - 1) + img->at(y, x + 1));
    g = center;
    b = 5 * img->at(y, x) - img->at(y - 2, x) - img->at(y - 1, x - 1) -
        img->at(y - 1, x + 1) - img->at(y + 2, x) - img->at(y + 1, x - 1) -
        img->at(y + 1, x + 1) + (img->at(y, x - 2) + img->at(y, x + 2)) / 2 +
        4 * (img->at(y - 1, x) + img->at(y + 1, x));
    r = r / 8;
    b = b / 8;
  }
  if (strcmp(is_color, "gb") == 0) {
    r = 5 * img->at(y, x) - img->at(y - 2, x) - img->at(y - 1, x - 1) -
        ...
        4 * (img->at(y - 1, x) + img->at(y + 1, x));
    g = center;
    b = 5 * img->at(y, x) - img->at(y, x - 2) - img->at(y - 1, x - 1) -
        img->at(y + 1, x - 1) - img->at(y - 1, x + 1) - img->at(y + 1, x + 1) -
        img->at(y, x + 2) + (img->at(y - 2, x) + img->at(y + 2, x)) / 2 +
        4 * (img->at(y, x - 1) + img->at(y, x + 1));
    r = r / 8;
...
ImageRGB CFA(ImageRaw &img, DEMOSAICING_MODE cfa_mode,
             BAYER_PATTERN bayer_pattern, uint8_t cfa_clip) {
    ...
    for (int x = 0; x < img_pad->getWidth() - 4 - 1; x += 2) {
      ...
      case BAYER_PATTERN_RGGB:
        ...
        case DEMOSAIC_MALVAR:
          ...
          cfa_img.at(y, x + 1) =
              malvar("gr", gr, y + 2, x + 3, img_pad, cfa_clip);
          cfa_img.at(y + 1, x) =
              malvar("gb", gb, y + 3, x + 2, img_pad, cfa_clip);
          cfa_img.at(y + 1, x + 1) =
              malvar("b", b, y + 3, x + 3, img_pad, cfa_clip);
          break;
      ...
      case BAYER_PATTERN_BGGR:
        ...
        case DEMOSAIC_MALVAR:
          ...
          cfa_img.at(y, x + 1) =
              malvar("gb", gb, y + 2, x + 3, img_pad, cfa_clip);
          cfa_img.at(y + 1, x) =
              malvar("gr", gr, y + 3, x + 2, img_pad, cfa_clip);
          cfa_img.at(y + 1, x + 1) =
              malvar("r", r, y + 3, x + 3, img_pad, cfa_clip);
          break;
        case DEMOSAIC_UNKNOWN:
        default:
          TRACE_DEBUG_LOG_ERROR("Unknown Pattern:%s\n", cfa_mode);
      ...
      case BAYER_PATTERN_GBRG:
        ...
        case DEMOSAIC_MALVAR:
          ...
          cfa_img.at(y, x + 1) =
              malvar("b", b, y + 2, x + 3, img_pad, cfa_clip);
          cfa_img.at(y + 1, x) =
              malvar("r", r, y + 3, x + 2, img_pad, cfa_clip);
          cfa_img.at(y + 1, x + 1) =
              malvar("gr", gr, y + 3, x + 3, img_pad, cfa_clip);
          break;
        case DEMOSAIC_UNKNOWN:
        default:
          TRACE_DEBUG_LOG_ERROR("Unknown Pattern:%s\n", cfa_mode);
      ...
      case BAYER_PATTERN_GRBG:
        ...
        case DEMOSAIC_MALVAR:
          ...
          cfa_img.at(y, x + 1) =
              malvar("r", r, y + 2, x + 3, img_pad, cfa_clip);
          cfa_img.at(y + 1, x) =
              malvar("b", b, y + 3, x + 2, img_pad, cfa_clip);
          cfa_img.at(y + 1, x + 1) =
              malvar("gb", gb, y + 3, x + 3, img_pad, cfa_clip);
          break;
        case DEMOSAIC_UNKNOWN:
        default:
          TRACE_DEBUG_LOG_ERROR("Unknown Pattern:%s\n", cfa_mode);
